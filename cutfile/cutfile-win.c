...
/* WARNING: DO NOT USE THIS CODE BECAUSE IT WORKS WRONG! */
...
/* csak linux rendszeren működőképes metódus, de windowsra a mingw32-gcc le tudja fordítani */
...
static unsigned char *read_whole_file(const char *file_name) {
    ...
    i = 0;
    //for (i = 0; i < 1; i++) {
        if (strstr(contents, bom[i].magic)) {
            status = strlen(contents) - bom[i].len;
            strncpy(contents, contents + bom[i].len, status);
            realloc(contents, status + 1);
            contents[status] = '\0';
    //        break;
        }
    //}
